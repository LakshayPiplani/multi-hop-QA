...
MODEL_DIR      = PROJECT_ROOT / "models" / "sft2"          # must match train_sft.py
...
for i in tqdm(range(0, len(examples), BATCH_SIZE)):
    ...
    for ex in batch_ex:
        ...
        prompt = serialize_example(
            ex, g, num_divergent=0
        ).split("CANDIDATE 1:")[0]  # cut out gold content
        prompt += "[INST]\n"        # ask model to answer
...
# ── Scores ──────────────────────────────────────────────────────────────────
results = squad_metric.compute(predictions=list(preds.values()),
